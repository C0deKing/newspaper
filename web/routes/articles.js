...
const updateArticle = (userId, articleId, headline, body, addLink1, addLink2, addLink3, s3Key, sequence, featured) => db.GetRecords("ap_updateArticle", 
																[userId || 0, articleId || 0, headline || "", body || "", 
																 addLink1 || "", addLink2 || "", addLink3 || "", s3Key || "", 
																 sequence || 0, featured || 0])
...
const article = async(req, res) => {
	if(req.user.isEditor || req.user.isAdmin){
		let [ [result], meta] = await updateArticle(req.user.id, req.params.id, req.body.headline, req.body.body, 
											...
											req.body.sequence, req.body.featured)
		res.send(result)
	}else {
